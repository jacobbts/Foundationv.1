(function () {
  ...
  function setRailAvailability() {
    if (!rail || !landingArea) return;
    const headerHeight = header?.offsetHeight || 0;
    const pastLanding = landingArea.getBoundingClientRect().bottom <= headerHeight;
    rail.classList.toggle('is-available', pastLanding);
    if (!pastLanding && rail.classList.contains('is-open')) closeRail();
  }
